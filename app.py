...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        sts = ['collected','collected','collected','missed','delayed']
        ...
        for day in range(30):
            ...
            for z in range(1, 10):
                rows.append((z, cid, z, random.choice(sts), 'Manual log', random.randint(1,6), random.choice(bts), random.choice(fls), d.strftime('%Y-%m-%d %H:%M:%S')))
...
function sendNotification(){const m=document.getElementById('notifMsg').value.trim(),t=document.getElementById('notifType').value;if(!m)return alert('Enter a message');fetch('/api/notifications/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:m,type:t})}).then(r=>r.json()).then(d=>{closeNotifModal();const e=document.getElementById('mlStatus');e.style.display='block';e.textContent=`✅ Notification sent to ${d.sent} residents!`;setTimeout(()=>e.style.display='none',3000)})}
</script></body></html>"""
...
<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"></script></head><body>
""" + MOBILE_HEADER + SIDEBAR_ADMIN + """
<div class="main-content"><div class="topbar"><div class="topbar-title">📊 Analytics & ML Predictions</div><button class="btn btn-primary btn-sm" onclick="runML()">🤖 Refresh ML</button></div><div class="page-content">
<div class="grid-2 mb-24"><div class="card"><div class="card-header"><div class="card-title">📅 Peak Waste Days</div></div><div class="chart-container"><canvas id="peakChart"></canvas></div></div><div class="card"><div class="card-header"><div class="card-title">📈 Collection Efficiency</div></div><div class="chart-container"><canvas id="effChart"></canvas></div></div></div>
<div class="card"><div class="card-header"><div><div class="card-title">🔮 7-Day Predictions</div><div class="card-subtitle">ML risk forecast per zone</div></div></div><div class="table-wrap"><table><thead><tr><th>Zone</th><th>Date</th><th>Level</th><th>Confidence</th><th>Action</th></tr></thead><tbody>{% for p in preds %}<tr><td><strong>{{ p.zone_name }}</strong></td><td>{{ p.predicted_date }}</td><td><span class="risk-dot risk-{{ p.waste_level }}"></span> <span class="badge {% if p.waste_level=='high' %}badge-red{% elif p.waste_level=='medium' %}badge-yellow{% else %}badge-green{% endif %}">{{ p.waste_level }}</span></td><td><div class="progress" style="width:80px;height:6px;display:inline-block;vertical-align:middle;margin-right:8px;"><div class="progress-bar" style="width:{{ (p.confidence_score*100)|int }}%"></div></div>{{ (p.confidence_score*100)|int }}%</td><td style="font-size:12px;">{% if p.waste_level=='high' %}Alert residents{% elif p.waste_level=='medium' %}Monitor{% else %}Normal{% endif %}</td></tr>{% endfor %}</tbody></table></div></div>
</div></div>
""" + JS_SIDEBAR + """
<script>
new Chart(document.getElementById('peakChart'),{type:'bar',data:{labels:{{ peak|tojson }}.map(d=>d.day),datasets:[{label:'Avg Volume (kg)',data:{{ peak|tojson }}.map(d=>d.avg),backgroundColor:{{ peak|tojson }}.map(d=>(d.day==='Fri'||d.day==='Sat')?'#ef4444':'#22c55e')}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}}}});
new Chart(document.getElementById('effChart'),{type:'line',data:{labels:{{ eff|tojson }}.map(d=>d.d.slice(5)),datasets:[{label:'Efficiency %',data:{{ eff|tojson }}.map(d=>d.pct),borderColor:'#16a34a',backgroundColor:'rgba(34,197,94,0.1)',tension:0.4,fill:true}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{min:0,max:100,ticks:{callback:v=>v+'%'}}}}});
function runML(){const b=event.target;b.disabled=true;b.innerHTML='⏳ Processing...';fetch('/api/run-ml',{method:'POST'}).then(r=>r.json()).then(d=>{b.disabled=false;b.innerHTML='🤖 Refresh ML';location.reload()})}
</script></body></html>"""
