...
def init_db():
    ...
    # Seed collection logs with VARIED statuses per log
...
def admin_dashboard():
    ...
    # FIXED: Per-resident collection status - get DIFFERENT logs per zone
    zone_residents = []
    ...
    for z in zones:
        ...
        if residents:
            # Get last 5 collection logs for this zone (different statuses)
            zone_logs = conn.execute("""
                SELECT DISTINCT status, collected_at 
            ...
            for i, r in enumerate(residents):
                # Assign different status to each resident based on available logs
                if zone_logs and i < len(zone_logs):
                    last_status = zone_logs[i]['status']
                    last_collected = zone_logs[i]['collected_at']
                elif zone_logs:
                    last_status = zone_logs[i % len(zone_logs)]['status']
...
def resident_dashboard():
    ...
    conn.close()
    return render_template_string(RESIDENT_HTML, household=hh, schedule=schedule, recent_collections=recent_collections, last_log=ll, notifs=notifs, my_reports=mr, today=today)
...
<div class="sidebar" id="sidebar">
  <div class="sidebar-brand"><div class="sidebar-logo"><div class="sidebar-logo-icon">♻️</div><div class="sidebar-logo-text"><h1>EcoTrack</h1><span>Barangay Waste System</span></div></div></div>
  <nav class="sidebar-nav">
    <div class="nav-section-label">Overview</div>
    <a href="/admin" class="nav-link active"><span class="icon">🏠</span> Dashboard</a>
    <a href="/admin/analytics" class="nav-link"><span class="icon">📊</span> Analytics & ML</a>
    <div class="nav-section-label">Management</div>
    <a href="/admin/zones" class="nav-link"><span class="icon">🗺️</span> Zones</a>
    <a href="/admin/schedules" class="nav-link"><span class="icon">📅</span> Schedules</a>
    <a href="/admin/users" class="nav-link"><span class="icon">👥</span> Users</a>
    <a href="/admin/reports" class="nav-link"><span class="icon">📋</span> Reports</a>
    <div class="nav-section-label">Communications</div>
    <a href="/admin/notifications" class="nav-link"><span class="icon">🔔</span> Notifications</a>
  </nav>
  <div class="sidebar-footer"><div class="sidebar-user"><div class="sidebar-user-avatar">{{ session.name[0] }}</div><div class="sidebar-user-info"><div class="sidebar-user-name">{{ session.name }}</div><div class="sidebar-user-role">{{ session.role }}</div></div><a href="/logout" title="Logout" style="color:var(--text-muted);font-size:16px;">⎋</a></div></div>
</div>
...
SIDEBAR_COLLECTOR = """
<div class="sidebar" id="sidebar">
  <div class="sidebar-brand"><div class="sidebar-logo"><div class="sidebar-logo-icon">♻️</div><div class="sidebar-logo-text"><h1>EcoTrack</h1><span>Collector Portal</span></div></div></div>
  <nav class="sidebar-nav"><div class="nav-section-label">Collector</div><a href="/collector" class="nav-link active"><span class="icon">🚛</span> My Routes</a></nav>
  <div class="sidebar-footer"><div class="sidebar-user"><div class="sidebar-user-avatar">{{ session.name[0] }}</div><div class="sidebar-user-info"><div class="sidebar-user-name">{{ session.name }}</div><div class="sidebar-user-role">Collector</div></div><a href="/logout" style="color:var(--text-muted);font-size:16px;">⎋</a></div></div>
</div>
<div class="sidebar-overlay" id="overlay" onclick="closeSidebar()"></div>
"""
...
SIDEBAR_RESIDENT = """
<div class="sidebar" id="sidebar">
  <div class="sidebar-brand"><div class="sidebar-logo"><div class="sidebar-logo-icon">♻️</div><div class="sidebar-logo-text"><h1>EcoTrack</h1><span>Resident Portal</span></div></div></div>
  <nav class="sidebar-nav"><div class="nav-section-label">My Home</div><a href="/resident" class="nav-link active"><span class="icon">🏡</span> Dashboard</a></nav>
  <div class="sidebar-footer"><div class="sidebar-user"><div class="sidebar-user-avatar">{{ session.name[0] }}</div><div class="sidebar-user-info"><div class="sidebar-user-name">{{ session.name }}</div><div class="sidebar-user-role">Resident</div></div><a href="/logout" style="color:var(--text-muted);font-size:16px;">⎋</a></div></div>
</div>
<div class="sidebar-overlay" id="overlay" onclick="closeSidebar()"></div>
"""
